...
function removeFirstOccurrences(str, value) {
  ...
  return str.substring(0, index).concat(str.substring(index + value.length));
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((acc, next) => (acc.length < next.length ? next : acc), '');
}
...
function getCardId(value) {
  ...
  return str.replaceAll("'", '').split(',').indexOf(value);
}
